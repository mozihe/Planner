...
KDTree::KDTree(const std::vector<cv::Point>& points) {
...
KDNode* KDTree::buildTree(const std::vector<cv::Point>& points, int depth) {
    ...
    std::sort(sorted_points.begin(), sorted_points.end(), [axis](const cv::Point& a, const cv::Point& b) {
...
void KDTree::kNearestNeighbors(KDNode* node, const cv::Point& query, int k, int depth, std::vector<std::pair<cv::Point, double>>& neighbors) {
    ...
    if (neighbors.size() < k || std::abs((axis == 0 ? query.x - node->point.x : query.y - node->point.y)) < neighbors.front().second) {
